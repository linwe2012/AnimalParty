...
float4 layerBlend(float4 back, float4 front)
{
    float a0 = front.a;
    float a1 = back.a;
...
float3 overlayBlend(float3 back, float3 front)
{
    return
        float3(
...
float4 GrabPixel(sampler2D tex, float4 uv)
{
    float4 pixel = tex2Dproj(tex, UNITY_PROJ_COORD(uv));
    return float4(pixel.rgb, 1);
}

float4 GrabPixelXY(sampler2D tex, float4 uv, float4 size, float kernelx, float kernely)
{
    float4 pixel = tex2Dproj(
        tex,
        UNITY_PROJ_COORD(
            float4(
                uv.x + size.x * kernelx,
...
float4 GetBlurInDir(v2f IN, float4 pixel, sampler2D tex, float4 size, float dirx, float diry)
{
    ...
    float steps = 1;
    for (float range = ITER_STEP; range <= radius; range += ITER_STEP)
    {
    ...
    float4 result = sum / steps;
    return float4(overlayBlend(result.rgb, color.rgb), result.a * visibility);
}